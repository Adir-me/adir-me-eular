#!/bin/sh
# ==============================================================================
#             E U L E R  -  A U T O M A T E D  O N E - C L I C K  S E T U P
# ==============================================================================
# This script installs all development dependencies, scaffolds the full Rust
# workspace, compiles the Euler binary, and creates an alias for easy execution.
# Supports both native Termux (Android) and standard Alpine Linux.
# ==============================================================================
...
echo "███████╗╚██████╔╝███████╗███████╗██║  ██║"
echo "╚══════╝ ╚═════╝ ╚══════╝╚══════╝╚═╝  ╚═╝"
echo "        AUTONOMOUS AI CODING AGENT BOOTSTRAP"
echo "${NC}"

echo "==> ${CYAN}[1/4] Detecting environment and package manager...${NC}"
if command -v apk >/dev/null 2>&1; then
    echo "==> ${GREEN}Detected Alpine Linux. Installing system packages...${NC}"
    sudo apk update && sudo apk add gcc musl-dev make rust cargo curl openssl-dev
elif command -v pkg >/dev/null 2>&1; then
    echo "==> ${GREEN}Detected Termux (Android). Installing packages...${NC}"
    pkg update -y && pkg install -y clang make rust curl openssl
else
    echo "==> ${YELLOW}No matching package manager found. Assuming rustc/cargo are present.${NC}"
fi

echo "==> ${CYAN}[2/4] Scaffolding source directory skeleton...${NC}"
mkdir -p euler-agent/src
cd euler-agent

# Writing Cargo.toml
echo "==> ${CYAN}Writing Cargo.toml...${NC}"
cat << 'EOF' > Cargo.toml
[package]
name = "euler"
version = "0.1.0"
...
# Writing src/main.rs
echo "==> ${CYAN}Writing src/main.rs...${NC}"
cat << 'EOF' > src/main.rs
...
# Writing src/llm.rs
echo "==> ${CYAN}Writing src/llm.rs...${NC}"
cat << 'EOF' > src/llm.rs
...
# Writing src/tools.rs
echo "==> ${CYAN}Writing src/tools.rs...${NC}"
cat << 'EOF' > src/tools.rs
use std::fs;
...
# Writing src/agent.rs
echo "==> ${CYAN}Writing src/agent.rs...${NC}"
cat << 'EOF' > src/agent.rs
use std::io::{self, Write};
use std::path::PathBuf;
...
echo "==> ${CYAN}[3/4] Compiling Rust release binary... (Optimized, lightweight, no-bloat)${NC}"
cargo build --release

echo "==> ${CYAN}[4/4] Creating shell alias or global symlink for execution...${NC}"
GLOBAL_DIR=""
for dir in "/usr/local/bin" "/usr/bin" "/bin" "$HOME/.bin" "$HOME/bin"; do
    if [ -d "$dir" ] && [ -w "$dir" ]; then
        GLOBAL_DIR="$dir"
        break
    fi
...
if [ -n "$GLOBAL_DIR" ]; then
    ln -sf "$PWD/target/release/euler" "$GLOBAL_DIR/euler" 2>/dev/null || cp "$PWD/target/release/euler" "$GLOBAL_DIR/euler"
    echo "==> ${GREEN}Global executable 'euler' installed successfully to $GLOBAL_DIR/euler!${NC}"
else
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! grep -q "alias euler=" "$SHELL_CONFIG"; then
            echo "alias euler=\"$PWD/target/release/euler\"" >> "$SHELL_CONFIG"
            echo "==> ${GREEN}Alias created in $SHELL_CONFIG! Restart your shell or run 'source $SHELL_CONFIG'${NC}"
        else
            echo "==> ${YELLOW}Alias already exists in $SHELL_CONFIG.${NC}"
        fi
    else
        echo "==> ${YELLOW}Could not detect .bashrc, .zshrc, or .profile. Executable can be run at:${NC}"
        echo "    $PWD/target/release/euler"
    fi
fi

echo ""
echo "=========================================================="
echo "      🎉 Euler Autonomous Agent Built Successfully! 🎉"
echo "=========================================================="
echo "To start Euler, run:"
echo "  euler"
echo ""
echo "Or direct execute:"
echo "  ./target/release/euler"
echo "=========================================================="
echo ""
